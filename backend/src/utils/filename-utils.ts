...
/**
 * Validate if a schema type exists
 * @param schemaType - The schema type to validate
 * @param availableSchemas - Array of available schema IDs
 * @returns True if valid, false otherwise
 */
export function isValidSchemaType(schemaType: string, availableSchemas: string[]): boolean {
  return availableSchemas.includes(schemaType);
}
...
export function getDisplayName(filename: string): string {
  ...
  return parsed ? parsed.name : filename;
}
